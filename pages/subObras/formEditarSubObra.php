...
?>
<form name="editarSubObra" id="editarSubObra" method="POST" action="procesarSubObra.php?accion=actualizar&codigoViejo=<?= $_REQUEST["idu"] ?>" onsubmit="return enviarRegistroSubObraEditar(event);">
		...
		<div class="form-group col-md-12">
			<div class="row" id="detalleEditar">
				...
				foreach ($fetchDetalles as $fetchDetalle) {
					...
					echo '<input  type="number" class="form-control" name="cantidad[]" id="cantidadEditar'.$contador.'" required="true"  maxlength="20" autocomplete="off" value="'.$fetchDetalle->cantidad_producto.'"/>';
					echo '</div>';
